...
bot = telebot.TeleBot("Токена нема((")
...
def handle_text(message):
    ...
    if message.from_user.id == 5041299186:
        bot.send_message(message.chat.id, f"*⚡ Тебе тут не место, {message.from_user.first_name}. Возможно, вы делали непозволительные вещи с ботом*", parse_mode="Markdown")
        ...
        bot.send_message(message.chat.id, f"*⚜ {message.from_user.first_name}, вы испорченны на {chance}%*", parse_mode="Markdown")
    elif user_input == "💻 Как пользоваться этим ботом?":
        bot.send_message(message.chat.id, f"*⚜ {message.from_user.first_name}, напишите любой многочлен или алгеброическое выражение, к примеру (a-3)(a+3).*", parse_mode="Markdown")
